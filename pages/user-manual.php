...
function cleanManualMeta($content) {
    return preg_replace('/^<!--MANUAL_META:.*?-->\s*/s', '', (string)$content);
}

function extractManualMetaCategory($content, $fallback = 'overview') {
    if (preg_match('/^<!--MANUAL_META:(.*?)-->\s*/s', (string)$content, $m)) {
        $meta = json_decode($m[1], true);
        if (is_array($meta) && !empty($meta['display_category'])) return $meta['display_category'];
    }
    return $fallback;
}

function normalizeManualRole($role) {
    return $role === 'all' ? '' : trim((string)$role);
}
...
function renderStaticManualSection($sectionId, $override, $defaultRole, $defaultCategory, $defaultContentSearch) {
    ...
    <section id="<?php echo htmlspecialchars($sectionId); ?>"
             ...
             data-section data-item-type="static" data-static-key="<?php echo htmlspecialchars($sectionId); ?>"
             data-role="<?php echo htmlspecialchars($role); ?>"
             data-category="<?php echo htmlspecialchars($category); ?>"
             data-display-order="<?php echo htmlspecialchars($override['display_order'] ?? 0); ?>"
...
// Handle Admin inline management for manual sections from this page.
if ($isAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && in_array(($_POST['manual_ajax'] ?? ''), ['static_save', 'section_delete', 'section_reorder'], true)) {
    header('Content-Type: application/json; charset=utf-8');
    try {
        $db = getDB()->getConnection();
        $ajaxAction = $_POST['manual_ajax'] ?? '';

        if ($ajaxAction === 'section_delete') {
            $itemType = $_POST['item_type'] ?? '';
            $itemId = $_POST['item_id'] ?? '';
            if ($itemType === 'static') {
                $sectionId = preg_replace('/[^a-zA-Z0-9\-_]/', '', $itemId);
                if ($sectionId === '') throw new Exception('ไม่พบรหัสหัวข้อ');
                $staticKey = getStaticManualKey($sectionId);
                $title = trim($_POST['title'] ?? $sectionId);
                $role = normalizeManualRole($_POST['role'] ?? '');
                $displayCategory = trim($_POST['display_category'] ?? 'overview');
                $displayOrder = (int)($_POST['display_order'] ?? 0);
                $content = trim($_POST['content'] ?? '<p>Deleted static section placeholder.</p>');
                $meta = json_encode(['display_category' => $displayCategory], JSON_UNESCAPED_UNICODE);
                $storedContent = "<!--MANUAL_META:$meta-->\n" . cleanManualMeta($content);

                $stmt = $db->prepare("SELECT id FROM user_manual WHERE category = :category LIMIT 1");
                $stmt->execute([':category' => $staticKey]);
                $existingId = $stmt->fetchColumn();
                if ($existingId) {
                    $stmt = $db->prepare("UPDATE user_manual SET title = :title, content = :content, role = :role, display_order = :display_order, is_active = 0 WHERE id = :id");
                    $stmt->execute([':title'=>$title, ':content'=>$storedContent, ':role'=>$role, ':display_order'=>$displayOrder, ':id'=>$existingId]);
                } else {
                    $stmt = $db->prepare("INSERT INTO user_manual (title, content, role, category, display_order, is_active) VALUES (:title, :content, :role, :category, :display_order, 0)");
                    $stmt->execute([':title'=>$title, ':content'=>$storedContent, ':role'=>$role, ':category'=>$staticKey, ':display_order'=>$displayOrder]);
                }
            } else {
                $id = (int)$itemId;
                if ($id <= 0) throw new Exception('ไม่พบรหัสหัวข้อ');
                $stmt = $db->prepare("UPDATE user_manual SET is_active = 0 WHERE id = :id AND (category IS NULL OR category NOT LIKE 'static:%')");
                $stmt->execute([':id' => $id]);
            }
        ...
        if ($ajaxAction === 'section_reorder') {
            $sections = json_decode($_POST['sections'] ?? '[]', true);
            if (!is_array($sections)) throw new Exception('รูปแบบข้อมูลลำดับไม่ถูกต้อง');
            $order = 10;
            foreach ($sections as $section) {
                $type = $section['type'] ?? '';
                $idRaw = $section['id'] ?? '';
                if ($type === 'static') {
                    $sectionId = preg_replace('/[^a-zA-Z0-9\-_]/', '', $idRaw);
                    if ($sectionId === '') continue;
                    $staticKey = getStaticManualKey($sectionId);
                    $title = trim($section['title'] ?? $sectionId);
                    $role = normalizeManualRole($section['role'] ?? '');
                    $displayCategory = trim($section['display_category'] ?? 'overview');
                    $content = trim($section['content'] ?? '<p></p>');
                    $meta = json_encode(['display_category' => $displayCategory], JSON_UNESCAPED_UNICODE);
                    $storedContent = "<!--MANUAL_META:$meta-->\n" . cleanManualMeta($content);
                    $stmt = $db->prepare("SELECT id FROM user_manual WHERE category = :category LIMIT 1");
                    $stmt->execute([':category' => $staticKey]);
                    $existingId = $stmt->fetchColumn();
                    if ($existingId) {
                        $stmt = $db->prepare("UPDATE user_manual SET display_order = :display_order, is_active = 1 WHERE id = :id");
                        $stmt->execute([':display_order' => $order, ':id' => $existingId]);
                    } else {
                        ...
                        $stmt->execute([':title'=>$title, ':content'=>$storedContent, ':role'=>$role, ':category'=>$staticKey, ':display_order'=>$order]);
                    }
                } else {
                    $id = (int)$idRaw;
                    if ($id > 0) {
                        $stmt = $db->prepare("UPDATE user_manual SET display_order = :display_order WHERE id = :id AND (category IS NULL OR category NOT LIKE 'static:%')");
                        $stmt->execute([':display_order' => $order, ':id' => $id]);
                    }
                }
                $order += 10;
            }
...
try {
    $db = getDB()->getConnection();
    $stmt = $db->prepare("SELECT * FROM user_manual WHERE category LIKE 'static:%'");
    $stmt->execute();
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $key = substr($row['category'], 7);
        if ((int)($row['is_active'] ?? 1) !== 1) {
            $staticManualDeleted[$key] = true;
            continue;
        }
        $content = $row['content'] ?? '';
        $row['content'] = cleanManualMeta($content);
        $row['display_category'] = extractManualMetaCategory($content, 'overview');
...
<head>
    ...
    <style>
        ...
        .manual-section:hover .manual-admin-controls { opacity: 1; transform: translateY(0); }
        .manual-admin-btn {
            width: 2rem;
            height: 2rem;
            display: inline-flex;
        ...
        .manual-admin-btn:hover { transform: translateY(-1px); background: #eff6ff; color: #2563eb; border-color: #bfdbfe; }
        .manual-admin-btn.delete:hover { background: #fef2f2; color: #dc2626; border-color: #fecaca; }
        .manual-admin-hint {
        ...
        <div class="main-content">
            <div class="manual-layout">
                ...
                <div class="manual-main">
                    ...
                    <!-- Content Sections -->
                    <?php if ($isAdmin): ?>
                    <div class="manual-admin-hint">
                        ผู้ดูแลระบบสามารถกดปุ่ม ↑ ↓ เพื่อสลับตำแหน่งการแสดง กดถังขยะเพื่อลบหัวข้อทีละส่วน และระบบจะบันทึกลำดับใหม่อัตโนมัติ
                    </div>
                    <?php endif; ?>
                    <div id="manualContainer">
                        <!-- 1. Role Map -->
                        <?php if (empty($staticManualDeleted['manual-role-map']) && isset($staticManualOverrides['manual-role-map'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-role-map'])): ?>
                        <section id="manual-role-map" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-role-map" data-role="all" data-category="overview" data-content="Role Map ผู้ประกอบการ กรรมการ ผู้ดูแลระบบ">
                            <h2>
                                <svg width="32" height="32" class="text-primary-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <!-- 2. Project Structure -->
                        <?php if (empty($staticManualDeleted['manual-project-structure']) && isset($staticManualOverrides['manual-project-structure'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-project-structure'])): ?>
                        <section id="manual-project-structure" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-project-structure" data-role="admin" data-category="overview" data-content="โครงสร้างโปรเจค โฟลเดอร์ api config database includes pages scripts">
                            <h2>
...
└── <span class="file">index.php</span>          <span class="count">← Entry Point</span></pre>
                        ...
                        <!-- 3. User Flow Diagram -->
                        <?php if (empty($staticManualDeleted['manual-flow']) && isset($staticManualOverrides['manual-flow'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-flow'])): ?>
                        <section id="manual-flow" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-flow" data-role="all" data-category="overview" data-content="Flow การใช้งาน User Flow diagram">
                            <h2>
...
flowchart TB
    ...
    D1 --> D2[Leaderboard]
                        ...
                        <!-- 4. Database Schema -->
                        <?php if (empty($staticManualDeleted['manual-database']) && isset($staticManualOverrides['manual-database'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-database'])): ?>
                        <section id="manual-database" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-database" data-role="admin" data-category="overview" data-content="โครงสร้างฐานข้อมูล Database Schema tables">
                            <h2>
                        ...
                        <!-- 5. API Endpoints -->
                        <?php if (empty($staticManualDeleted['manual-api']) && isset($staticManualOverrides['manual-api'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-api'])): ?>
                        <section id="manual-api" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-api" data-role="admin" data-category="overview" data-content="API Endpoints upload download save assessment">
                            <h2>
                        ...
                        <!-- 6. Scoring Formula -->
                        <?php if (empty($staticManualDeleted['manual-formulas']) && isset($staticManualOverrides['manual-formulas'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-formulas'])): ?>
                        <section id="manual-formulas" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-formulas" data-role="admin" data-category="overview" data-content="formulas สูตรการคำนวณ คะแนน Pillar H1 I2 C3 M4">
                            <h2>
                                <svg width="32" height="32" class="text-primary-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <?php if (empty($staticManualDeleted['manual-pillars']) && isset($staticManualOverrides['manual-pillars'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-pillars'])): ?>
                        <section id="manual-pillars" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-pillars" data-role="all" data-category="overview" data-content="4 Pillars H1 I2 C3 M4 โครงสร้างน้ำหนัก ระดับคะแนน HICM">
                            <h2>
                        ...
                        <!-- 8. Personas -->
                        <?php if (empty($staticManualDeleted['manual-personas']) && isset($staticManualOverrides['manual-personas'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-personas'])): ?>
                        <section id="manual-personas" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-personas" data-role="all" data-category="usage" data-content="ตัวละคร Persona Admin Auditor Company CEO โปรไฟล์">
                            <h2>
                        ...
                        <!-- 9. Status Flow -->
                        <?php if (empty($staticManualDeleted['manual-status']) && isset($staticManualOverrides['manual-status'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-status'])): ?>
                        <section id="manual-status" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-status" data-role="all" data-category="usage" data-content="สถานะการประเมิน draft submitted under_review evaluated completed">
                            <h2>
...
stateDiagram-v2
    ...
    evaluated --> completed: Admin อนุมัติ
                        ...
                        <!-- 10. Steps: Company -->
                        <?php if (empty($staticManualDeleted['manual-steps-company']) && isset($staticManualOverrides['manual-steps-company'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-steps-company'])): ?>
                        <section id="manual-steps-company" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-steps-company" data-role="company" data-category="usage" data-content="ขั้นตอนผู้ประกอบการ Company กรอกแบบประเมิน แนบหลักฐาน">
                            <h2>
                        ...
                        <!-- 11. Steps: Auditor -->
                        <?php if (empty($staticManualDeleted['manual-steps-auditor']) && isset($staticManualOverrides['manual-steps-auditor'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-steps-auditor'])): ?>
                        <section id="manual-steps-auditor" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-steps-auditor" data-role="auditor" data-category="usage" data-content="ขั้นตอนกรรมการ Auditor ประเมิน ให้คะแนน ตรวจหลักฐาน">
                            <h2>
                        ...
                        <!-- 12. Steps: Admin -->
                        <?php if (empty($staticManualDeleted['manual-steps-admin']) && isset($staticManualOverrides['manual-steps-admin'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-steps-admin'])): ?>
                        <section id="manual-steps-admin" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-steps-admin" data-role="admin" data-category="usage" data-content="ขั้นตอน Admin ผู้ดูแลระบบ จัดการผู้ใช้ ตั้งค่า">
                            <h2>
                        ...
                        <!-- 13. Steps: CEO -->
                        <?php if (empty($staticManualDeleted['manual-steps-ceo']) && isset($staticManualOverrides['manual-steps-ceo'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-steps-ceo'])): ?>
                        <section id="manual-steps-ceo" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-steps-ceo" data-role="ceo" data-category="usage" data-content="ขั้นตอน CEO ผู้บริหาร ดูรายงาน Leaderboard สรุปผล">
                            <h2>
                        ...
                        <?php if (empty($staticManualDeleted['manual-tech']) && isset($staticManualOverrides['manual-tech'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-tech'])): ?>
                        <section id="manual-tech" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-tech" data-role="admin" data-category="overview" data-content="เทคโนโลยีที่ใช้ Technology Stack PHP SQL JavaScript Mermaid PDF Export">
                            <h2>
                        ...
                        <?php if (empty($staticManualDeleted['manual-examples']) && isset($staticManualOverrides['manual-examples'])): ?>
                        ...
                        <?php elseif (empty($staticManualDeleted['manual-examples'])): ?>
                        <section id="manual-examples" class="manual-section reveal-on-scroll static-manual-section" data-section data-item-type="static" data-static-key="manual-examples" data-role="all" data-category="usage" data-content="ตัวอย่างการใช้งาน Use Cases Examples Scenario">
                            <h2>
                        ...
                        <?php foreach ($allManualItems as $item): ?>
                            <section id="manual-item-<?php echo $item['id']; ?>" 
                                     ...
                                     data-section data-item-type="dynamic" data-dynamic-id="<?php echo (int)$item['id']; ?>" 
                                     data-role="<?php echo $item['role']; ?>"
                                     data-category="<?php echo $item['category'] ?? ''; ?>"
                                     data-display-order="<?php echo (int)($item['display_order'] ?? 0); ?>"
    ...
    <script>
        ...
        function getSectionIdentity(section) {
            if (section.classList.contains('static-manual-section')) {
                return { type: 'static', id: section.getAttribute('data-static-key') || section.id };
            }
            return { type: 'dynamic', id: section.getAttribute('data-dynamic-id') || (section.id || '').replace('manual-item-', '') };
        }
        ...
        function getSectionTitle(section) {
            ...
            clone.querySelectorAll('button, .section-badge, .manual-admin-controls').forEach(el => el.remove());
            return clone.textContent.trim() || section.id;
        }

        function getSectionContent(section) {
            const contentNode = section.querySelector('.static-editable-content, .manual-content-rendered');
            return contentNode ? contentNode.innerHTML.trim() : '';
        }

        function normalizeSectionOrders() {
            document.querySelectorAll('#manualContainer .manual-section').forEach((section, index) => {
                const current = parseInt(section.getAttribute('data-display-order') || '0', 10);
                if (!current) section.setAttribute('data-display-order', String((index + 1) * 10));
            });
        }

        function sortManualSectionsByOrder() {
            const container = document.getElementById('manualContainer');
            if (!container) return;
            const sections = Array.from(container.querySelectorAll('.manual-section'));
            sections.sort((a, b) => {
                const ao = parseInt(a.getAttribute('data-display-order') || '9999', 10);
                const bo = parseInt(b.getAttribute('data-display-order') || '9999', 10);
                return ao - bo;
            });
            const noResults = document.getElementById('noResults');
            sections.forEach(section => container.insertBefore(section, noResults));
        }

        function addAdminSectionControls() {
            if (!IS_ADMIN) return;
            document.querySelectorAll('#manualContainer .manual-section').forEach(section => {
                if (section.querySelector('.manual-admin-controls')) return;
                const controls = document.createElement('div');
                controls.className = 'manual-admin-controls';
                controls.innerHTML = `
                    <button type="button" class="manual-admin-btn" title="เลื่อนขึ้น" onclick="moveManualSection('${section.id}', -1)">↑</button>
                    <button type="button" class="manual-admin-btn" title="เลื่อนลง" onclick="moveManualSection('${section.id}', 1)">↓</button>
                    <button type="button" class="manual-admin-btn delete" title="ลบหัวข้อนี้" onclick="deleteManualSection('${section.id}')">🗑</button>
                `;
        ...
        function collectSectionOrder() {
            return Array.from(document.querySelectorAll('#manualContainer .manual-section')).map((section, index) => {
                const ident = getSectionIdentity(section);
                section.setAttribute('data-display-order', String((index + 1) * 10));
                return {
                    type: ident.type,
                    id: ident.id,
                    title: getSectionTitle(section),
                    role: section.getAttribute('data-role') || '',
                    display_category: section.getAttribute('data-category') || 'overview',
                    content: getSectionContent(section)
                };
            });
        }

        function persistSectionOrder(showAlert = false) {
            if (!IS_ADMIN) return Promise.resolve();
            const formData = new FormData();
            formData.append('manual_ajax', 'section_reorder');
            formData.append('sections', JSON.stringify(collectSectionOrder()));
            return fetch(window.location.href, { method: 'POST', body: formData })
                .then(r => r.json())
                .then(data => {
                    if (!data.success) throw new Error(data.error || 'Unknown error');
                    if (showAlert) alert('บันทึกลำดับการแสดงเรียบร้อยแล้ว');
                })
                .catch(err => alert('บันทึกลำดับล้มเหลว: ' + err.message));
        }

        function moveManualSection(sectionId, direction) {
            const section = document.getElementById(sectionId);
            const container = document.getElementById('manualContainer');
            if (!section || !container) return;
            const sections = Array.from(container.querySelectorAll('.manual-section')).filter(el => el.offsetParent !== null);
            const index = sections.indexOf(section);
            const swapWith = sections[index + direction];
            if (!swapWith) return;
            const noResults = document.getElementById('noResults');
            if (direction < 0) container.insertBefore(section, swapWith);
            else container.insertBefore(swapWith, section);
            if (noResults) container.appendChild(noResults);
            persistSectionOrder(false);
        }

        function deleteManualSection(sectionId) {
            const section = document.getElementById(sectionId);
            if (!section) return;
            const title = getSectionTitle(section);
            if (!confirm('ต้องการลบหัวข้อ "' + title + '" ใช่หรือไม่?')) return;
            const ident = getSectionIdentity(section);
            const formData = new FormData();
            formData.append('manual_ajax', 'section_delete');
            formData.append('item_type', ident.type);
            formData.append('item_id', ident.id);
            formData.append('title', title);
            formData.append('role', section.getAttribute('data-role') || '');
            formData.append('display_category', section.getAttribute('data-category') || 'overview');
            formData.append('display_order', section.getAttribute('data-display-order') || '0');
            formData.append('content', getSectionContent(section));
            fetch(window.location.href, { method: 'POST', body: formData })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert('ลบหัวข้อล้มเหลว: ' + (data.error || 'Unknown error'));
                    }
                })
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            sortManualSectionsByOrder();
            addAdminSectionControls();
        ...
        function openEditModal(itemId) {
            ...
            if (typeof itemId === 'string' && itemId.startsWith('static:')) {
                ...
                roleSelect.value = (section.getAttribute('data-role') === 'all' ? '' : (section.getAttribute('data-role') || ''));
