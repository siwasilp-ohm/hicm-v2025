<?php
/**
 * HICM V2025 - Full Demo Data Seeder (3 Auditors)
 * ============================================================
 * สร้าง demo data ครบ loop:
 *   1. เปิดรอบการประเมินใหม่ (assessment period)
 *   2. จับคู่กรรมการ 3 คน ต่อ 1 บริษัท
 *   3. บริษัทประเมินตนเอง (self-assessment) ครบ 60 ตัวชี้วัด
 *   4. กรรมการประเมินบริษัท (auditor evaluation) ครบ 60 ตัวชี้วัด × 3 คน
 *   5. คำนวณคะแนนรวม + กำหนดระดับ HICM
 * ============================================================
 * Usage: php scripts/seed_demo_3auditors.php
 */
...
// CONFIGURATION
// ======================================================================
$PERIOD_YEAR   = 2570;  // B.E. 2570 = A.D. 2027
$PERIOD_NAME   = 'รอบประเมินประจำปี 2570 (Demo 3 กรรมการ)';
$NUM_AUDITORS  = 3;     // จำนวนกรรมการต่อบริษัท

echo "============================================================\n";
echo "  HICM V2025 - Full Demo Data Seeder (3 กรรมการ/บริษัท)\n";
echo "============================================================\n\n";
...
function randomScore($companyProfile = 'medium') {
    ...
    switch ($companyProfile) {
        case 'excellent':  $weights = [1, 2, 5, 25, 67]; break;
        case 'good':       $weights = [1, 3, 10, 40, 46]; break;
        case 'performing': $weights = [2, 5, 20, 43, 30]; break;
        case 'developing': $weights = [3, 10, 35, 35, 17]; break;
        case 'emerging':   $weights = [8, 20, 35, 27, 10]; break;
        default:           $weights = [3, 8, 25, 40, 24]; break;
    }
...
function auditorVariation($selfScore) {
    ...
    if ($idx === false) $idx = 2;
    $r = mt_rand(1, 100);
    if ($r <= 45) return $selfScore;
    elseif ($r <= 65) return $scores[max(0, $idx - 1)];
    elseif ($r <= 85) return $scores[min(4, $idx + 1)];
    elseif ($r <= 93) return $scores[max(0, $idx - 2)];
    else return $scores[min(4, $idx + 2)];
}
...
// ======================================================================
// STEP 1: Create Assessment Period (new period)
// ======================================================================
...
if ($existingPeriod) {
    ...
    $pdo->prepare("UPDATE assessment_periods SET status = 'open', is_active = 1, show_auditor_results = 1,
        ...
        WHERE id = ?")->execute([
        '2027-01-01', '2027-12-31', '2027-06-30', '2027-07-01', '2027-10-31', '2027-11-15', $periodId
    ]);
} else {
    $stmt = $pdo->prepare("
    ...
    $stmt->execute([
        $PERIOD_YEAR,
        $PERIOD_NAME,
        'รอบการประเมิน Demo สำหรับทดสอบ flow ครบ loop (3 กรรมการ/บริษัท) - ปี 2570',
        '2027-01-01',
        '2027-12-31',
        '2027-06-30',
        '2027-07-01',
...
foreach ($companies as $idx => $company) {
    ...
    echo "\n  [{$idx}] {$companyName} (profile={$profile})\n";
    ...
    // Set evaluator_id on assessment
    ...
    // --- Auditor scores (3 auditors × 60 indicators) ---
    ...
    echo "      กรรมการ: [" . implode(', ', $assignedAuditors) . "] → Final={$updated['final_score']} Level {$level} ({$levelNames[$level]})\n";
}
...
// ======================================================================
echo "\n" . str_repeat("═", 80) . "\n";
echo "  ✅ สรุปผล Demo Data (3 กรรมการ/บริษัท)\n";
echo str_repeat("═", 80) . "\n";
...
echo "    - assessments:                 " . count($companies) . " รายการ\n";
